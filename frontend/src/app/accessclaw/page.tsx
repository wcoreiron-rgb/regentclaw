...
export default function AccessClawPage() {
  ...
  const load = async () => {
    ...
    try {
      const [s, f, p] = await Promise.all([
        apiFetch<any>('/accessclaw/stats'),
        apiFetch<any[]>('/accessclaw/findings'),
        apiFetch<any[]>('/accessclaw/providers'),
      ]);
  ...
  const handleScan = async () => {
    ...
    setScanMsg(null);
    try {
      const res = await apiFetch<any>('/accessclaw/scan', { method: 'POST' });
      await load();
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-3xl font-bold flex items-center gap-3" style={{ color: 'var(--rc-text-1)' }}>
            <Key className="text-amber-400" /> AccessClaw
          </h1>
          <p className="mt-1 text-sm" style={{ color: 'var(--rc-text-2)' }}>
            Privileged access management - over-permissioned accounts, MFA gaps, and stale credentials across your identity stack.
          </p>
        </div>
      ...
      <div className="rounded-xl border p-4" style={{ background: 'var(--rc-bg-surface)', borderColor: 'var(--rc-border)' }}>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-semibold flex items-center gap-2" style={{ color: 'var(--rc-text-1)' }}>
            <Plug className="w-4 h-4 text-amber-400" /> Connected Providers
          </h2>
      ...
      <div className="flex gap-2 flex-wrap">
        {['all', 'critical', 'high', 'medium', 'low'].map(sev => (
          <button key={sev} onClick={() => setFilterSev(sev)}
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold capitalize transition-colors ${filterSev === sev ? 'bg-amber-600 text-white' : 'hover:opacity-80'}`}
