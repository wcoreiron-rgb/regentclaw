...
export default function AttackPathClawPage() {
  ...
  const load = async () => {
    ...
    try {
      const [s, f, p] = await Promise.all([
        apiFetch<any>('/attackpathclaw/stats'),
        apiFetch<any[]>('/attackpathclaw/findings'),
        apiFetch<any[]>('/attackpathclaw/providers'),
      ]);
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          {[
            ...
            { label: 'Paths', val: stats?.attack_paths ?? '\u2014', c: 'text-orange-400 bg-orange-900/20 border-orange-800' },
