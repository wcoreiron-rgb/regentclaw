...
export default function AutomationClawPage() {
  ...
  const load = async () => {
    ...
    try {
      const [s, f, p] = await Promise.all([
        apiFetch<any>('/automationclaw/stats'),
        apiFetch<any[]>('/automationclaw/findings'),
        apiFetch<any[]>('/automationclaw/providers'),
      ]);
  ...
  const handleScan = async () => {
    ...
    setScanMsg(null);
    try {
      const res = await apiFetch<any>('/automationclaw/scan', { method: 'POST' });
      await load();
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          {[
            ...
            { label: 'Failed', val: stats?.failed_automations ?? '\u2014', c: 'text-orange-400 bg-orange-900/20 border-orange-800' },
