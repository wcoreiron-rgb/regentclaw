...
import { Network, RefreshCw, Plug, ChevronDown, ChevronRight } from 'lucide-react';
...
export default function NetClawPage() {
  ...
  const load = async () => {
    ...
    try {
      const [s, f, p] = await Promise.all([
        apiFetch<any>('/netclaw/stats'),
        apiFetch<any[]>('/netclaw/findings'),
        apiFetch<any[]>('/netclaw/providers'),
      ]);
  ...
  const handleScan = async () => {
    ...
    setScanMsg(null);
    try {
      const res = await apiFetch<any>('/netclaw/scan', { method: 'POST' });
      await load();
