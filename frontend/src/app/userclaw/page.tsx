...
import { UserCheck, RefreshCw, Plug, ChevronDown, ChevronRight } from 'lucide-react';
...
export default function UserClawPage() {
  ...
  const load = async () => {
    ...
    try {
      const [s, f, p] = await Promise.all([
        apiFetch<any>('/userclaw/stats'),
        apiFetch<any[]>('/userclaw/findings'),
        apiFetch<any[]>('/userclaw/providers'),
      ]);
  ...
  const handleScan = async () => {
    ...
    setScanMsg(null);
    try {
      const res = await apiFetch<any>('/userclaw/scan', { method: 'POST' });
      await load();
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-4">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          {[
            { label: 'Total', val: stats?.total_findings ?? stats?.total ?? '\u2014', c: 'text-pink-400 bg-pink-900/20 border-pink-800' },
            { label: 'Critical', val: stats?.by_severity?.critical ?? stats?.critical ?? '\u2014', c: 'text-red-400 bg-red-900/20 border-red-800' },
            { label: 'Anomalies', val: stats?.anomalies ?? '\u2014', c: 'text-orange-400 bg-orange-900/20 border-orange-800' },
      ...
      <div className="rounded-xl border p-4" style={{ background: 'var(--rc-bg-surface)', borderColor: 'var(--rc-border)' }}>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-semibold flex items-center gap-2" style={{ color: 'var(--rc-text-1)' }}>
            <Plug className="w-4 h-4 text-pink-400" /> Connected Providers
          </h2>
      ...
      <div className="flex gap-2 flex-wrap">
        {['all', 'critical', 'high', 'medium', 'low'].map(sev => (
          <button key={sev} onClick={() => setFilterSev(sev)}
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold capitalize transition-colors ${filterSev === sev ? 'bg-pink-600 text-white' : 'hover:opacity-80'}`}
