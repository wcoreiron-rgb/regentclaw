...
import { Package, RefreshCw, Plug, ChevronDown, ChevronRight } from 'lucide-react';
...
export default function SaaSClawPage() {
  ...
  const load = async () => {
    ...
    try {
      const [s, f, p] = await Promise.all([
        apiFetch<any>('/saasclaw/stats'),
        apiFetch<any[]>('/saasclaw/findings'),
        apiFetch<any[]>('/saasclaw/providers'),
      ]);
  ...
  const handleScan = async () => {
    ...
    setScanMsg(null);
    try {
      const res = await apiFetch<any>('/saasclaw/scan', { method: 'POST' });
      await load();
