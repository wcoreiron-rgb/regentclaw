...
import { Settings, RefreshCw, Plug, ChevronDown, ChevronRight } from 'lucide-react';
...
export default function ConfigClawPage() {
  ...
  const load = async () => {
    ...
    try {
      const [s, f, p] = await Promise.all([
        apiFetch<any>('/configclaw/stats'),
        apiFetch<any[]>('/configclaw/findings'),
        apiFetch<any[]>('/configclaw/providers'),
      ]);
  ...
  const handleScan = async () => {
    ...
    setScanMsg(null);
    try {
      const res = await apiFetch<any>('/configclaw/scan', { method: 'POST' });
      await load();
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-3xl font-bold flex items-center gap-3" style={{ color: 'var(--rc-text-1)' }}>
            <Settings className="text-sky-400" /> ConfigClaw
          </h1>
          <p className="mt-1 text-sm" style={{ color: 'var(--rc-text-2)' }}>
            Configuration drift detection - baseline deviations, hardening failures, and CIS benchmark violations across cloud and on-prem.
          </p>
        </div>
        <div className="flex items-center gap-3 flex-wrap">
          {[
            ...
            { label: 'Drifted', val: stats?.drifted_configs ?? '\u2014', c: 'text-orange-400 bg-orange-900/20 border-orange-800' },
