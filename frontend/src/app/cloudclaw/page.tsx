...
import { Cloud, RefreshCw, Plug, ChevronDown, ChevronRight } from 'lucide-react';
...
export default function CloudClawPage() {
  ...
  const load = async () => {
    ...
    try {
      const [s, f, p] = await Promise.all([
        apiFetch<any>('/cloudclaw/stats'),
        apiFetch<any[]>('/cloudclaw/findings'),
        apiFetch<any[]>('/cloudclaw/providers'),
      ]);
  ...
  const handleScan = async () => {
    ...
    setScanMsg(null);
    try {
      const res = await apiFetch<any>('/cloudclaw/scan', { method: 'POST' });
      await load();
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-3xl font-bold flex items-center gap-3" style={{ color: 'var(--rc-text-1)' }}>
            <Cloud className="text-sky-400" /> CloudClaw
          </h1>
          <p className="mt-1 text-sm" style={{ color: 'var(--rc-text-2)' }}>
            Cloud security posture management across AWS, Azure, and GCP — misconfigurations, exposed resources, and compliance drift.
          </p>
        </div>
        <div className="flex items-center gap-3 flex-wrap">
          {[
            { label: 'Total',    val: stats?.total_findings ?? stats?.total ?? '—',  c: 'text-sky-400 bg-sky-900/20 border-sky-800' },
            { label: 'Critical', val: stats?.by_severity?.critical ?? stats?.critical ?? '—', c: 'text-red-400 bg-red-900/20 border-red-800' },
            { label: 'Open',     val: stats?.open_count ?? stats?.open ?? '—',      c: 'text-orange-400 bg-orange-900/20 border-orange-800' },
      ...
      <div className="rounded-xl border p-4" style={{ background: 'var(--rc-bg-surface)', borderColor: 'var(--rc-border)' }}>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-semibold flex items-center gap-2" style={{ color: 'var(--rc-text-1)' }}>
            <Plug className="w-4 h-4 text-sky-400" /> Connected Providers
          </h2>
      ...
      <div className="flex gap-2 flex-wrap">
        {['all', 'critical', 'high', 'medium', 'low'].map(sev => (
          <button key={sev} onClick={() => setFilterSev(sev)}
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold capitalize transition-colors ${filterSev === sev ? 'bg-sky-600 text-white' : 'hover:opacity-80'}`}
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-dashed p-8 text-center" style={{ background: 'var(--rc-bg-surface)', borderColor: 'var(--rc-border)' }}>
          ...
          <p className="text-sm mb-3" style={{ color: 'var(--rc-text-2)' }}>Connect a provider above or click Run Scan to load demo findings.</p>
        </div>
      ) : (
                  ...
                  <div className="px-4 pb-4 border-t pt-3 space-y-3" style={{ borderColor: 'var(--rc-border)' }}>
                    ...
                    {f.remediation && (
                      <div className="rounded-lg p-3" style={{ background: 'var(--rc-bg-elevated)' }}>
                        ...
                        {f.remediation_effort && <span className="text-xs mt-1 inline-block text-yellow-400">Effort: {f.remediation_effort.replace('_', ' ')}</span>}
                      </div>
                    )}
