...
import { Database, RefreshCw, Plug, ChevronDown, ChevronRight } from 'lucide-react';
...
export default function DataClawPage() {
  ...
  const load = async () => {
    ...
    try {
      const [s, f, p] = await Promise.all([
        apiFetch<any>('/dataclaw/stats'),
        apiFetch<any[]>('/dataclaw/findings'),
        apiFetch<any[]>('/dataclaw/providers'),
      ]);
  ...
  const handleScan = async () => {
    ...
    setScanMsg(null);
    try {
      const res = await apiFetch<any>('/dataclaw/scan', { method: 'POST' });
      await load();
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          {[
            ...
            { label: 'Exposed PII', val: stats?.exposed_pii ?? '\u2014', c: 'text-orange-400 bg-orange-900/20 border-orange-800' },
