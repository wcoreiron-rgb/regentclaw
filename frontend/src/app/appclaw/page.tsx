...
import { Code, RefreshCw, Plug, ChevronDown, ChevronRight } from 'lucide-react';
...
export default function AppClawPage() {
  ...
  const load = async () => {
    ...
    try {
      const [s, f, p] = await Promise.all([
        apiFetch<any>('/appclaw/stats'),
        apiFetch<any[]>('/appclaw/findings'),
        apiFetch<any[]>('/appclaw/providers'),
      ]);
  ...
  const handleScan = async () => {
    ...
    setScanMsg(null);
    try {
      const res = await apiFetch<any>('/appclaw/scan', { method: 'POST' });
      await load();
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-4">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          {[
            { label: 'Total', val: stats?.total_findings ?? stats?.total ?? '\u2014', c: 'text-blue-400 bg-blue-900/20 border-blue-800' },
      ...
      <div className="rounded-xl border p-4" style={{ background: 'var(--rc-bg-surface)', borderColor: 'var(--rc-border)' }}>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-semibold flex items-center gap-2" style={{ color: 'var(--rc-text-1)' }}>
            <Plug className="w-4 h-4 text-blue-400" /> Connected Providers
          </h2>
      ...
      <div className="flex gap-2 flex-wrap">
        {['all', 'critical', 'high', 'medium', 'low'].map(sev => (
          <button key={sev} onClick={() => setFilterSev(sev)}
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold capitalize transition-colors ${filterSev === sev ? 'bg-blue-600 text-white' : 'hover:opacity-80'}`}
